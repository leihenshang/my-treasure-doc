...
type UserToken struct {
	...
	TokenExpire  time.Time  `json:"tokenExpire" gorm:"column:token_expire;type:timestamp;comment:token超时时间"`
	LoginIp      string     `json:"loginIp" gorm:"column:login_ip;type:varchar(100);default:'';comment:最后登陆ip地址"`
	LoginTime    time.Time  `json:"loginTime" gorm:"column:login_time;type:timestamp;comment:最后登陆时间"`
	LoginOutTime *time.Time `json:"loginOutTime" gorm:"column:login_out_time;type:timestamp;comment:退出登陆时间"`
}
