...
`RegisterRootUser`）。该账号创建时会被标记「强制修改密码」（`td_user.require_pwd_reset = 1`），提醒机制如下：

1. 登录响应返回 `requirePwdReset: true`；
2. 管理端登录后**弹出一次**修改密码弹窗，用户可关闭忽略——单次忽略后本次登录不再打扰；
3. 「单次」由前端的仅内存标记保证，刷新页面不会重复弹窗；
4. 重新登录时，只要还没改密（标记仍为 `1`），会再次收到提醒；改密成功后标记清除，之后登录不再提醒；
5. 后端 `middleware.RequireAdmin()` **不再拦截**未改密的管理请求，改密与否只影响前端提醒，不影响后台使用。

启动日志不再打印明文密码。

### 修改默认管理员密码

- **管理端修改（推荐）**：按弹窗提示输入原密码与新密码即可；也可随时在右上角头像下拉菜单选择「修改密码」。
...
- **命令行重置**（忘记密码或无法登录时，密码规则与登录入口一致，8-16 位）：
  ...
  该子命令不清除 `require_pwd_reset` 标记，因此重置后首次登录仍会提醒修改密码。
...
关于表结构变更，`AutoMigrate` 行为是「只加列、不删列」：

- **新增字段**：启动时自动对已有表执行 `ALTER TABLE ... ADD COLUMN`，存量行取默认值/ NULL；
  但新增 `NOT NULL` 且无 `default` 的字段在 SQLite 上可能加列失败，新建字段应允许 NULL 或带默认值。
- **删除字段**：不会自动 DROP 列，旧库会残留列定义。若残留列是 `NOT NULL` 且无默认值，
  新代码插入时不再写入该列会直接报 `NOT NULL constraint failed`。因此删字段的改动
  必须在 `module/user/global/db.go` 的 `migrateDbTable()` 里补一段 `Migrator().HasColumn(...)`
  判断 + `Migrator().DropColumn(...)` 清理（参考已移除的 `td_blog_bookmark.public_id`）。
- 修改列类型/长度受 SQLite `ALTER` 能力限制，需要时优先考虑重建库或手动迁移。
