...
- 本仓库是 Go 1.22 单模块项目（`fastduck/treasure-doc`），核心服务位于 `module/user`，技术栈为 Gin、GORM、MySQL；Redis 可选。
...
- 首次运行前按需从 `config.example.toml` 创建本地 `config.toml` 并配置可用的 MySQL。
...
- 模型的 `TableName()` 当前硬编码为 `td_*`。不要假设修改 `mysql.tablePrefix` 会自动改变已有模型表名。
...
6. 为纯逻辑优先添加单元测试；涉及 MySQL 的流程若无法自动测试，至少保证 `go test ./...` 和构建通过，并说明未做集成验证。
